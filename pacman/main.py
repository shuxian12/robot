...
class Wall(GameObject):
    def __init__(self, in_surface, x, y, in_size: int, in_color=(0, 0, 100)):
...
class GameRenderer:
    def __init__(self, in_width: int, in_height: int):
        ...
        self._lives = 1
        self._score = 0
        self._score_cookie_pickup = 1
        self._score_ghost_eaten = 20
        self._score_powerup_pickup = 3
        ...
        self.respawn1 = pygame.USEREVENT + 4
        self.respawn2 = pygame.USEREVENT + 5
        self.respawn3 = pygame.USEREVENT + 6
        self.respawn4 = pygame.USEREVENT + 7
        self._modes = [
            (7, 20),
            (7, 20),
            (5, 20),
    ...
    def _handle_events(self):
        for event in pygame.event.get():
            ...
            if event.type == self.respawn1:
                self._ghosts[0].set_position(352, 416)
                pygame.time.set_timer(self.respawn1, 0)
            if event.type == self.respawn2:
                self._ghosts[1].set_position(384, 416)
                pygame.time.set_timer(self.respawn2, 0)
            if event.type == self.respawn3:
                self._ghosts[2].set_position(416, 416)
                pygame.time.set_timer(self.respawn3, 0)
            if event.type == self.respawn4:
                self._ghosts[3].set_position(448, 416)
                pygame.time.set_timer(self.respawn4, 0)
        ...
        for ghost in ghosts:
            ...
            if collides and ghost in game_objects:
                if self._renderer.is_kokoro_active():
                    if ghost == ghosts[0]:
                        self._renderer._ghosts[0].set_position(0, 0)
                        self._renderer._ghosts[0].set_direction(Direction.NONE)
                        pygame.time.set_timer(self._renderer.respawn1, 15000)
                    elif ghost == ghosts[1]:
                        self._renderer._ghosts[1].set_position(0, 0)
                        self._renderer._ghosts[1].set_direction(Direction.NONE)
                        pygame.time.set_timer(self._renderer.respawn2, 15000)
                    elif ghost == ghosts[2]:
                        self._renderer._ghosts[2].set_position(0, 0)
                        self._renderer._ghosts[2].set_direction(Direction.NONE)
                        pygame.time.set_timer(self._renderer.respawn3, 15000)
                    else:
                        self._renderer._ghosts[3].set_position(0, 0)
                        self._renderer._ghosts[3].set_direction(Direction.NONE)
                        pygame.time.set_timer(self._renderer.respawn4, 15000)
